    ...
    def prepare_inputs(self):
        ...
        if self.host_params["needs_tar"]:
            ...
            if not self.needs_conda_tarfile:
               ...
               # conda_pack_command = "conda pack -p /eos/home-a/athachay/pkgs/condaInstalls --ignore-editable-packages --ignore-missing-files -o %s --compress-level 5 --n-threads 12" % self.conda_tarfile # compression level of 5 seems like a good balance of speed and size reduction: compression of 1 gives a tarfile of size 463MB, while 5 gives 413MB in 30s, while 9 gives 409MB in 91s 
