...
for variation, sync_info in VARIATIONS.items():
    ...
    for x, y in result.items():
        if not math.isclose(y, sync_info[x], rel_tol=1e-06):
            logger.warning(
                "[sync.py] Variation '%s' has target of %.3g for field '%s' but sync exercise found value of %.3g." %
                (variation, sync_info[x], x, y))
            pass_current_variation = False
            pass_variations = False

    if pass_current_variation:
        logger.info(
            "[sync.py] 'SUCCESS' : variation '%s' has expected number of events & mean/std dev of mgg distribution." %
            variation)

if pass_variations:
    logger.info(
        "[sync.py] 'SUCCESS' : All systematics with independent collections are properly synced.")
    ...
    for variation, target in sync_info.items():
        ...
        for x, y in result.items():
            if not math.isclose(y, target[x], rel_tol=1e-06):
                logger.warning(
                    "[sync.py] Weight '%s', variation '%s' has target of %.3g for field '%s' but sync exercise found value of %.3g." %
                    (weight, variation, target[x], x, y))
                pass_current_weight = False
                pass_weights = False

    if pass_current_weight:
        logger.info(
            "[sync.py] 'SUCCESS' : weight '%s' has expected mean and std dev for the following variations '%s'." %
            (weight,
             ",".join(
                 list(
                     sync_info.keys()))))
...
if pass_weights:
    logger.info(
        "[sync.py] 'SUCCESS' : All weight systematics are properly synced.")
